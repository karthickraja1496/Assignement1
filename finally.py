...
def databases():
    ...
    try:
        ...
        createcomment='''create table Commentinfo(commentid varchar(200) primary key,
                                    videoid varchar(200),
...
def migrate_commentinfo_data(mycursor, mydb, client, channel_name):
    ...
    for index, row in df.iterrows():
        query = '''INSERT INTO Commentinfo(commentid, videoid, commenttext, commentauthor, commentpublished,Channel_Id,Channel_Name)
                VALUES(%s, %s, %s, %s, %s,%s,%s)'''
        value = (row['commentid'], row['videoid'], row['commenttext'], row['commentauthor'], row['commentpublished'],row['Channel_Id'],row['Channel_Name'])
...
def migrate_data_to_mysql(channel_name):
    try:
        ...
        # Call the functions to migrate data for the specified channel_id
